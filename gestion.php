        ...
        //var_dump($dataH);
            function getUserIP() {
                ...
                if (isset($_SERVER['HTTP_CLIENT_IP']))
                    ...
                    $ipaddress = 'Adresse IP non reconnu';
                return $ipaddress;
            }
        if($dataH[0]->cbModification == null){
            $qtrestant = $dlqtte - $dlqtep;
            $histo = array(['qtpreparer'=>$dlqtep, 'qtrestant'=>$qtrestant, 'date'=>$date, 'ip'=> getUserIP()]);
            ...
            array_push($data_uncompressed,['qtpreparer'=>$dlqtep, 'qtrestant'=>$qtrestant, 'date'=>$date, 'ip'=> getUserIP()]);
                                                ...
                                                <?php foreach($data as $val){
                                                    ...
                                                    <tr class="">
                                                      <form method="post">
                                                        ...
                                                        <td><?php echo (empty($val->DL_Design)?$val->AR_Design:$val->DL_Design); ?><input type="hidden" name="<?php if($val->statut == 9){echo 'numBonAnn[]';}else{echo 'numBont[]';} ?>" class="form-control numBon" id="numBon" value="<?php if(isset($numBon)){echo $numBon;} ?>"></td>
